...
def test_one_instance_observes_and_steps(env_path, tmp_path):
    ...
    env = AnimalEnv(env_path, paths, 0, BASE_PORT, 0)
    try:
        visual, vels = env.reset()
        visual_shape, vels_shape = observation_shapes(EnvConfig())
        assert visual.shape == visual_shape and visual.dtype == np.uint8
        assert vels.shape == vels_shape

        first_time = vels[-1]
        for _ in range(5):
            (_, vels), reward, _, done = env.step(3)
...
def test_light_training(env_path, tmp_path):
    ...
    try:
        ...
        runner = Runner(vec_env, ppo, config, SilentLogger(), show=False)
        before = torch.cat([p.detach().reshape(-1).clone() for p in ppo.agent.parameters()])
        runner.train(str(tmp_path / "last.pt"), str(tmp_path / "best.pt"))
        after = torch.cat([p.detach().reshape(-1).clone() for p in ppo.agent.parameters()])
    finally:
        vec_env.close()

    assert runner.epoch == config.max_epochs
...
def test_light_evaluation(env_path, tmp_path):
    ...
    envs = [AnimalEnv(env_path, paths, 0, BASE_PORT + 20, 0)]
